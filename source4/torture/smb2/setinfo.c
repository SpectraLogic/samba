...
/* 
 * Check for execution against Likewise. If target is likewise then 
 * tag and count the errors rather than bailing out.
 */
#define STATUS_RESET	status = status2 = NT_STATUS_OK 
#define CHECK_LIKEWISE(tctx, fmt, ...)					\
	do {								\
		if (!torture_setting_bool(tctx, "likewise", false)) {	\
			torture_result(tctx, TORTURE_FAIL, fmt, ##__VA_ARGS__);\
			goto done;					\
		} else {						\
			torture_warning(tctx, "LIKEWISE: "	\
				fmt, ##__VA_ARGS__);			\
			if (++err_cnt >= torture_failures) {      	\
				goto done;                      	\
			} else {                                    	\
				STATUS_RESET;                   	\
			}						\
...
#define RECREATE_FILE(fname) do { \
	...
	if (!NT_STATUS_IS_OK(status)) { \
		ret = false; \
		CHECK_LIKEWISE(tctx, "(%s) ERROR: open of %s failed (%s)\n", \
...
#define CHECK_VALUE(call, stype, field, value) do { \
	...
	if (NT_STATUS_IS_OK(status) && NT_STATUS_IS_OK(status2) && finfo2.stype.out.field != value) { \
		...
		CHECK_LIKEWISE(tctx, "(%s) %s - %s/%s should be 0x%x - 0x%x\n",\
			__location__, call_name, #stype, #field, \
			(unsigned int)value, \
			(unsigned int)finfo2.stype.out.field); \
	}} while (0)
...
#define CHECK_STATUS(status, correct) do { \
	...
	CHECK_VALUE(SMB2_ALL_INFORMATION, all_info2, size, 4096);
